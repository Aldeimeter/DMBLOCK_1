...
class HandleTxs():
    ...
    def tx_is_valid_wrapped(self, tx: Transaction) -> bool:
        try:
            return self.tx_is_valid(tx)
        except exceptions.PhaseOneError:
            return False
    ...
    def handler(self, possible_txs: List[Transaction]) -> List[Transaction]:
        ...
        while True:
            ...
            for tx in validated_txs:
                possible_txs.remove(tx)
            if len(new_possible_txs) == len(possible_txs):
                break
